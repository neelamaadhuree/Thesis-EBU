...
file_path = './data/blendsignal.csv'
...
def plot_comparison_single_axis(poison_rate, sample_sizes):
    for idx, samples in enumerate(sample_sizes):
        fig, ax = plt.subplots()
        fig.suptitle(f'Comparison of Methods for Poison Rate {poison_rate}, Sample {samples}', fontsize=16)
        
        # Filter data for the given poison rate and number of samples
        filtered_data = data[(data['Poison Rate'] == poison_rate) & (data['Number of Samples'] == samples)]
        
        # Include "No Unlearning" data
        no_unlearning_data = data[(data['Method'] == 'No Unlearning') & (data['Poison Rate'] == poison_rate)].copy()
