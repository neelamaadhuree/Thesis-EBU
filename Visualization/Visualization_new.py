...
# Load the CSV files
signal_trigger = pd.read_csv('./Visualization/data/SignalTriggerraw.csv')
patch_trigger = pd.read_csv('./Visualization/data/PatchTriggerraw.csv')
checker_trigger = pd.read_csv('./Visualization/data/CheckerTriggerraw.csv')
freq_trigger = pd.read_csv('./Visualization/data/FreqTriggerraw.csv')
...
def create_combined_plots_100(signal_df, patch_df, checker_df, freq_df):
    ...
    plt.title('Train and Test Accuracy (First 100 Epochs)',fontsize=25,pad=20)
    plt.legend()
    plt.tight_layout()
    plt.show()
    ...
    plt.xlabel('Epoch',fontsize=20,labelpad=20)
    plt.ylabel('ASR',fontsize=20,labelpad=20)
    plt.title('Train and Test ASR (First 100 Epochs)',fontsize=25,pad=20)
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
    plt.ylim(0,105)
    plt.legend()
