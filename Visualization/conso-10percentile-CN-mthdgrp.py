...
#bar_colors = [ '#7ACCC8', ]
#dot_colors = [ '#008080', ]
# Vibrant Bar Colors (Hex Codes):
#bar_colors = [ '#92A8D1', '#88B04B']

# Corresponding Darker Dot Colors (Hex Codes):
#dot_colors = [ '#2A4D69', '#44693D']
# Vibrant Bar Colors (Hex Codes):
bar_colors = [ '#F9D5E5','#7ACCC8','#92A8D1', '#88B04B','#FFAB91']

# Corresponding Darker Dot Colors (Hex Codes):
dot_colors = ['#E75480','#008080','#2A4D69', '#44693D','#FF5722']
...
for i, sample in enumerate(samples):
    ...
    ax.bar(r, df['ASR'], width=bar_width, label=f'ASR- {int(sample)} samples',color=bar_colors[colorchc])
    ax.scatter(r, df['Test Acc'], s=150, marker='o', label=f'Test Acc-{int(sample)} Samples',color=dot_colors[colorchc])
    colorchc+=1
...
#ax.set_title('10th Percentile ASR and Test Accuracy by Number of Samples and Method', fontsize=20)
